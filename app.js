...
function renderIndicatorLineChart(el,labels,series,{maxValue=null,percent=false,decimals=0,height=340,fitWidth=false,emphasis=false}={}){
  ...
  async function ensureFeedbacksLoaded(force=false){
    ...
    }catch(err){console.error('Feedbacks indisponíveis',err);state.feedbackCache[key]=[];if(state.currentView==='feedbacks')toast('Não foi possível carregar os feedbacks. Confira se a migração V2.25.1 foi executada.');return[];}
