...
function renderBusinessDaysDistribution(body,rows,type){
  if(!body)return;body.innerHTML=(rows||[]).map(r=>{const q=r[type],incomplete=type==='service'&&q?.dailyComplete===false,has=safe(q?.total)>0,note=n=>incomplete?'—':has?`<span class="business-days-note-share">${fmtInt(q[`notes${n}`])}<small>${(safe(q[`notes${n}`])/q.total*100).toLocaleString('pt-BR',{minimumFractionDigits:1,maximumFractionDigits:1})}%</small></span>`:'—';return `<tr><td><strong>${escapeHtml(r.label)}</strong>${incomplete?'<small class="muted" style="display:block;margin-top:4px">Reimporte o CSV operacional para sincronizar o histórico diário</small>':''}</td><td>${note(1)}</td><td>${note(2)}</td><td>${note(3)}</td><td>${note(4)}</td><td>${note(5)}</td><td>${incomplete?'—':has?fmtInt(q.total):'—'}</td></tr>`;}).join('')||'<tr><td colspan="7" class="muted">Sem dados para comparar.</td></tr>';
}
function renderBusinessDaysIndicators(squads){
  ...
  const rows=compareIds.map(id=>businessDaysMonthSummary(squads,id,cutoff)),baseRow=rows[rows.length-1],prev=rows.length>1?rows[rows.length-2]:null,incompleteServiceRows=rows.filter(r=>r.service?.dailyComplete===false);
...
function renderQualityGroupedBarChart(el,labels,rows){
  ...
  const grid=[0,25,50,75,100].map(v=>{const yy=y(v);return `<line x1="${p.l}" y1="${yy}" x2="${w-p.r}" y2="${yy}" class="grid-line"/><text x="8" y="${yy+4}" class="axis-label">${v}%</text>`}).join('');let bars='';series.forEach((ser,si)=>{(ser.values||[]).forEach((v,i)=>{if(v==null)return;const xx=x0(i)+si*(barW+2),yy=y(v);bars+=`<rect x="${xx}" y="${yy}" width="${barW}" height="${Math.max(0,base-yy)}" rx="3" fill="${ser.color}" class="quality-bar chart-series-shape" data-series-index="${si}"></rect>`})});
...
function renderIndicatorLineChart(el,labels,series,{maxValue=null,percent=false,decimals=0,height=340,fitWidth=false,emphasis=false}={}){
  ...
  async function persistQualityMonth(m,fileName){
    ...
    const {error:de}=await state.supabase.from('quality_person_daily_metrics').delete().eq('squad_month_id',m.dbId);if(de)throw new Error(`Não foi possível gravar a nova estrutura de qualidade. Execute MIGRACAO_V2.27.0.sql. ${de.message||''}`.trim());
  ...
  function syncHistoricalServiceDailyFromCsv(pending,selectedId){
    const changed=[],monthKeys=new Set();let techsPatched=0,daysPatched=0,skippedMismatch=0;
    for(const code of pending?.codes||[]){const squad=state.squads?.[code];if(!squad)continue;for(const id of pending.months||[]){if(id===selectedId)continue;const mon=squad.months?.[id];if(!mon)continue;const source=serviceCsvMonthTechMap(pending.rows,id,code);if(!source.size)continue;for(const t of mon.technicians||[]){const src=source.get(nameLinkKey(t.name));if(!src)continue;const sourceMatchesMonthly=[5,4,3,2,1].every(n=>safe(src[`notes${n}`])===safe(t[`notes${n}`]));if(!sourceMatchesMonthly){skippedMismatch++;continue}const current={notes5:0,notes4:0,notes3:0,notes2:0,notes1:0};for(const d of t.daily||[])for(const n of [5,4,3,2,1])current[`notes${n}`]+=safe(d[`notes${n}`]);const needsPatch=[5,4,3,2,1].some(n=>safe(current[`notes${n}`])!==safe(t[`notes${n}`]));if(!needsPatch)continue;for(const d of t.daily||[]){const sday=src.daily.get(safe(d.day));for(const n of [5,4,3,2,1])d[`notes${n}`]=safe(sday?.[`notes${n}`]);daysPatched++;}techsPatched++;monthKeys.add(`${code}|${id}`);changed.push({code,id,month:mon,tech:t});}}
    }
    return{changed,monthsPatched:monthKeys.size,techsPatched,daysPatched,skippedMismatch};
  }
