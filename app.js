(() => {
  ...
  function technicianPeriodAggregate(squad,techName,start=state.analysisStartDate,end=state.analysisEndDate){
    const result={att:0,notes5:0,notes4:0,notes3:0,notes2:0,notes1:0,totalEval:0,avg:0,evalPct:0,daily:[],excludeFromGroupCount:false,excludedMonths:[],countedMonths:[]};
    ...
    for(const id of analysisMonthIds()){
      ...
      if(fullCompetence){
        ...
        agg={att:safe(t.att),notes5:safe(t.notes5),notes4:safe(t.notes4),notes3:safe(t.notes3),notes2:safe(t.notes2),notes1:safe(t.notes1)};
      }else{
        agg=aggregateDailyRows(rows);
      }
      ...
      result.att+=safe(agg.att);result.notes5+=safe(agg.notes5);result.notes4+=safe(agg.notes4);result.notes3+=safe(agg.notes3);result.notes2+=safe(agg.notes2);result.notes1+=safe(agg.notes1);
    }
    ...
    result.evalPct=result.att?roundTo(result.totalEval/result.att,4):0;
...
function renderIndicatorLineChart(el,labels,series,{maxValue=null,percent=false,decimals=0,height=340,fitWidth=false,emphasis=false,axisFormatter=null,valueFormatter=null}={}){
  ...
  function scoreRules(m,options={}){
    ...
    return{refAtt:auto.refAtt,refTotalEval:auto.refTotalEval,refAvg:auto.refAvg,refEvalPct:auto.refEvalPct,baseRefAtt:auto.refAtt,baseRefTotalEval:auto.refTotalEval,bonusAtt:safe(saved.bonusAtt)||20,bonusTotalEval:safe(saved.bonusTotalEval)||30,bonusAvg:safe(saved.bonusAvg)||40,bonusEvalPct:safe(saved.bonusEvalPct)||35,manualCount:0,progress:1,elapsedDays,totalDays,activeCount:auto.activeCount||0,countedCount:auto.countedCount||0,excludedCount:auto.excludedCount||0,attSource:'Total do Squad ÷ técnicos considerados (0 casas)',evalSource:'Avaliações do Squad ÷ técnicos considerados (0 casas)',avgSource:'Média dos técnicos considerados truncada (2 casas)',pctSource:'Média dos técnicos considerados arredondada (4 casas)'};
  }
