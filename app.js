...
function renderBusinessDaysDistribution(body,rows,type){
  if(!body)return;body.innerHTML=(rows||[]).map(r=>{const q=r[type],has=safe(q?.total)>0,note=n=>has?`<span class="business-days-note-share">${fmtInt(q[`notes${n}`])}<small>${(safe(q[`notes${n}`])/q.total*100).toLocaleString('pt-BR',{minimumFractionDigits:1,maximumFractionDigits:1})}%</small></span>`:'—';return `<tr><td><strong>${escapeHtml(r.label)}</strong></td><td>${note(1)}</td><td>${note(2)}</td><td>${note(3)}</td><td>${note(4)}</td><td>${note(5)}</td><td>${has?fmtInt(q.total):'—'}</td></tr>`;}).join('')||'<tr><td colspan="7" class="muted">Sem dados para comparar.</td></tr>';
}
function renderBusinessDaysIndicators(squads){
  ...
  const rows=compareIds.map(id=>businessDaysMonthSummary(squads,id,cutoff)),baseRow=rows[rows.length-1],prev=rows.length>1?rows[rows.length-2]:null;
  const attDelta=prev&&prev.att?((baseRow.att/prev.att)-1)*100:null,evalDelta=prev?(baseRow.evalRate-prev.evalRate)*100:null,prdDelta=prev&&baseRow.product.total&&prev.product.total?(baseRow.product.lowPct-prev.product.lowPct)*100:null,cmpDelta=prev&&baseRow.company.total&&prev.company.total?(baseRow.company.lowPct-prev.company.lowPct)*100:null;
