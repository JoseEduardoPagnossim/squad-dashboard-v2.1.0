...
function orgTechnicianDailyRows(){
  const localRows=buildOrgTechnicianDailyOverviewFromState();
  // O quadro de Evolucao diaria e exclusivo do Admin Geral. Para esse perfil,
  // priorizamos os daily_metrics que ja vieram carregados junto das competencias.
  // Isso evita depender do RPC organizacional para abrir o grafico e mantem a
  // mesma fonte usada pelos demais indicadores diarios da tela.
  if((window.APP_CONFIG?.mode||'demo')==='demo'||isSuperAdmin()){
    return localRows.length?localRows:(state.orgTechnicianDailyOverview||[]);
  }
  return (state.orgTechnicianDailyOverview||[]).length?state.orgTechnicianDailyOverview:localRows;
}
