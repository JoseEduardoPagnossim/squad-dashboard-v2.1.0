(() => {
  ...
  async function enterApp(user){
    state.user=user;
    state.userDirectoryLoaded=false;state.userDirectory=[];state.gameRankingCache={};state.gameRankingLoading={};state.feedbackCache={};state.feedbackLoading={};state.feedbackEditor=null;state.myFeedbacks=null;state.myFeedbackLoading=false;
  ...
  function applyPermissions(){
    ...
    const analyticalView=['individual','team','indicators'].includes(state.currentView);if($('.month-control'))$('.month-control').classList.toggle('hidden',analyticalView||['users','profile','help','my-feedbacks'].includes(state.currentView));if($('#analysisDateControl'))$('#analysisDateControl').classList.toggle('hidden',!['individual','team'].includes(state.currentView));syncAnalysisDateControls();
  ...
  function resetAnalysisRange(force=false){
    const bounds=importedDateBounds();if(!bounds.max){state.analysisStartDate=null;state.analysisEndDate=null;syncAnalysisDateControls();return;}
  ...
  function periodTechniciansForSquad(squad,start=state.analysisStartDate,end=state.analysisEndDate){
    const names=new Map();for(const id of analysisMonthIds()){const m=squad?.months?.[id];for(const t of m?.technicians||[]){const key=nameLinkKey(t.name);if(!names.has(key))names.set(key,t.name);}}
  ...
  function showView(name,adminSection=null){
    if((name==='admin'||name==='users'||name==='feedbacks')&&!isAdmin())return;
    ...
    const titles={individual:'Meu desempenho',team:'Visão do Squad',indicators:'Indicadores',feedbacks:'Feedbacks',users:'Usuários',admin:adminTitles[state.adminSection]||'Gestão',profile:'Meu perfil','my-feedbacks':'Meus feedbacks',help:'Como usar'};
    ...
    $('.month-control').classList.toggle('hidden',analytical||name==='users'||name==='profile'||name==='my-feedbacks'||name==='help');
  ...
  function renderTeam(){
    ...
    const avgPoints=periodRows.length?meanOf(periodRows,t=>t.periodPoints):0,aboveCount=periodRows.filter(t=>safe(t.periodPoints)>avgPoints).length,ratio=periodRows.length?aboveCount/periodRows.length:0,periodStatus=periodRows.length?(ratio>=.5?'ACIMA':'ABAIXO'):'—';
    const teamResultBox=$('#teamResult')?.closest('.team-result');$('#teamResult').textContent=periodStatus;if(teamResultBox){teamResultBox.classList.toggle('above',periodStatus==='ACIMA');teamResultBox.classList.toggle('below',periodStatus==='ABAIXO');teamResultBox.classList.toggle('neutral',!['ACIMA','ABAIXO'].includes(periodStatus))}$('#teamAtt').textContent=fmtInt(totals.att);$('#teamAttGoal').textContent=fmtInt(periodGoal);$('#teamEval').textContent=fmtInt(totals.eval);$('#teamPct').textContent=fmtPct(totals.evalPct);$('#teamPctGoal').textContent=fmtPct(cfg.teamGoalEvalPct);$('#teamAttBar').style.width=clamp(attProgress*100,0,100)+'%';$('#teamPctBar').style.width=clamp(evalProgress*100,0,100)+'%';$('#teamAttNote').textContent=`${analysisRangeLabel()} • ${fmtPct(attProgress)} da meta proporcional`;$('#teamPctNote').textContent=totals.evalPct>=cfg.teamGoalEvalPct?`${((totals.evalPct-cfg.teamGoalEvalPct)*100).toLocaleString('pt-BR',{minimumFractionDigits:1,maximumFractionDigits:1})} p.p. acima da meta`:`Faltam ${((cfg.teamGoalEvalPct-totals.evalPct)*100).toLocaleString('pt-BR',{minimumFractionDigits:1,maximumFractionDigits:1})} p.p. para a meta`;$('#teamHeroTitle').textContent=`Squad ${state.squadCode} • ${analysisRangeLabel()}`;
    $('#teamAvgPoints').textContent=fmtNum(avgPoints);$('#teamAvgPointsNote').textContent=`${aboveCount} de ${periodRows.length} acima da média do período`;$('#teamStatusAudit').textContent=periodRows.length?`${aboveCount} de ${periodRows.length} técnicos acima de ${fmtNum(avgPoints)} pts no período • ${fmtPct(ratio)} • mínimo 50%`:'Sem técnicos com produção no período';
    $('#teamLeaderboard').innerHTML=periodRows.map(t=>`<div class="leader-item ${t.periodRank===1?'top1':''}"><div class="place">#${t.periodRank||'—'}</div><div class="name">${escapeHtml(t.name)}</div><div class="metric"><span>Atend.</span><strong>${fmtInt(t.att)}</strong></div><div class="metric"><span>Notas 5</span><strong>${fmtInt(t.notes5)}</strong></div><div class="metric hide-md"><span>% Aval.</span><strong>${fmtPct(t.evalPct)}</strong></div><div class="metric points"><span>Pontos período*</span><strong>${fmtNum(t.periodPoints)}</strong></div><div><span class="status ${String(t.periodStatus).toUpperCase()==='ACIMA'?'above':'below'}">${escapeHtml(t.periodStatus||'—')}</span></div></div>`).join('')||'<div class="muted">Sem dados no período selecionado.</div>';
...
function renderIndicators(){
  ...
  const dailySeries=[];
  squads.forEach((squad,sIndex)=>{const periodRows=periodTechniciansForSquad(squad);const totals=deriveTotals(periodRows);totalAtt+=totals.att;totalEval+=totals.eval;totalNotes5+=periodRows.reduce((sum,t)=>sum+safe(t.notes5),0);totalPoints+=periodRows.reduce((sum,t)=>sum+safe(t.periodPoints),0);totalTechRecords+=periodRows.length;const avg=periodRows.length?meanOf(periodRows,t=>t.periodPoints):0;const above=periodRows.filter(t=>safe(t.periodPoints)>avg).length,below=Math.max(0,periodRows.length-above);totalAbove+=above;totalBelow+=below;periodRows.forEach(t=>{distinctTechs.add(`${squad.code}|${nameLinkKey(t.name)}`);const key=`${squad.code}|${nameLinkKey(t.name)}`,prev=techTotals.get(key)||{name:t.name,squad:squad.code,points:0,months:1,evalPct:0};prev.points=safe(t.periodPoints);prev.evalPct=safe(t.evalPct);techTotals.set(key,prev);totalWorkedDays+=(t.daily||[]).filter(d=>isBusinessDateIso(d.date)).length;});if(periodRows.length){squadMonthTotal++;if(above/periodRows.length>=.5)squadMonthHits++;}
...
function buildTechnicianHistory(squad,ids){
  const techMap=new Map();(ids||[]).forEach(id=>(squad?.months?.[id]?.technicians||[]).forEach(t=>{const key=historyTechKey(t);if(!techMap.has(key))techMap.set(key,{key,name:t.name});}));const entities=[...techMap.values()].sort((a,b)=>String(a.name).localeCompare(String(b.name),'pt-BR')),attendance=entities.map((e,i)=>({name:titleWords(e.name),color:HISTORY_COLORS[i%HISTORY_COLORS.length],values:[]})),daily=entities.map((e,i)=>({name:titleWords(e.name),color:HISTORY_COLORS[i%HISTORY_COLORS.length],values:[]})),evaluation=entities.map((e,i)=>({name:titleWords(e.name),color:HISTORY_COLORS[i%HISTORY_COLORS.length],values:[]})),totals=[];
  (ids||[]).forEach(id=>{let monthTotal=0;entities.forEach((e,idx)=>{const rows=dailyRowsForTechnician(squad,e.name).filter(d=>`${d.year}-${String(d.month).padStart(2,'0')}`===id),agg=aggregateDailyRows(rows),workDays=Math.max(1,new Set(rows.filter(d=>isBusinessDateIso(d.date)).map(d=>d.date)).size);attendance[idx].values.push(rows.length?agg.att:null);daily[idx].values.push(rows.length?agg.att/workDays:null);evaluation[idx].values.push(rows.length?agg.evalPct*100:null);monthTotal+=agg.att;});totals.push(monthTotal);});
  daily.unshift({name:'Média Squad',color:'var(--accent2)',dashed:true,values:(ids||[]).map((id,ix)=>{const vals=daily.map(s=>s.values[ix]).filter(v=>v!=null);return vals.length?meanOf(vals,x=>x):null;})});evaluation.unshift({name:'Total geral',color:'var(--accent2)',dashed:true,values:(ids||[]).map((id,ix)=>{let att=0,ev=0;entities.forEach(e=>{const rows=dailyRowsForTechnician(squad,e.name).filter(d=>`${d.year}-${String(d.month).padStart(2,'0')}`===id),agg=aggregateDailyRows(rows);att+=agg.att;ev+=agg.totalEval;});return att?(ev/att)*100:null;})});return{ids,labels:(ids||[]).map(shortHistoryMonth),totals,attendance,daily,evaluation,entityCount:entities.length};
}
...
function renderIndicatorLineChart(el,labels,series,{maxValue=null,percent=false,decimals=0,height=340,fitWidth=false,emphasis=false}={}){
  ...
  function generatedFeedbackFromSnapshot(snap={}){
    const monthLabel=`${snap.monthName||MONTHS_PT[Math.max(0,safe(snap.month)-1)]||'Competência'} de ${snap.year||''}`,status=snap.status||'SEM STATUS',rankText=snap.rank?`${snap.rank}ª posição`:'posição não definida',technicianName=titleWords(snap.technicianName||'Técnico');
    const summaryParts=[`Em ${monthLabel}, ${technicianName} registrou ${fmtInt(snap.att)} atendimentos, ${fmtInt(snap.totalEval)} avaliações (${fmtPct(snap.evalPct)} dos atendimentos), nota média ${safe(snap.avg).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})} e ${fmtNum(snap.points)} pontos, encerrando a competência na ${rankText} do Squad ${snap.squad||'—'} com status ${status}.`];
    ...
    if(snap.goalAtt&&safe(snap.att)>=safe(snap.goalAtt))strengths.push(`Atingiu a meta mensal de atendimentos (${fmtInt(snap.att)} de ${fmtInt(snap.goalAtt)}).`);
    if(snap.goalNotes5&&safe(snap.notes5)>=safe(snap.goalNotes5))strengths.push(`Atingiu a meta mensal de notas 5 (${fmtInt(snap.notes5)} de ${fmtInt(snap.goalNotes5)}).`);
    if(snap.goalEvalPct&&safe(snap.evalPct)>=safe(snap.goalEvalPct))strengths.push(`Manteve taxa de avaliação acima da meta do Squad (${fmtPct(snap.evalPct)} frente a ${fmtPct(snap.goalEvalPct)}).`);
    if(safe(snap.avg)>=safe(snap.refs?.avg)&&safe(snap.totalEval)>0)strengths.push(`Nota média de ${safe(snap.avg).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}, igual ou superior à referência da equipe (${safe(snap.refs?.avg).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}).`);
    if(snap.previous&&safe(snap.points)>safe(snap.previous.points)*1.05)strengths.push('Apresentou evolução relevante de pontuação em relação à competência anterior.');
    ...
    const improvements=[];
    if(snap.goalAtt&&safe(snap.att)<safe(snap.goalAtt))improvements.push(`Elevar o volume de atendimentos para alcançar a meta mensal de ${fmtInt(snap.goalAtt)}.`);
    if(snap.goalNotes5&&safe(snap.notes5)<safe(snap.goalNotes5))improvements.push(`Aumentar a quantidade de notas 5 para alcançar a meta mensal de ${fmtInt(snap.goalNotes5)}.`);
    if(snap.goalEvalPct&&safe(snap.evalPct)<safe(snap.goalEvalPct))improvements.push(`Reforçar a solicitação de avaliações ao final dos atendimentos; a taxa ficou em ${fmtPct(snap.evalPct)}, abaixo da meta de ${fmtPct(snap.goalEvalPct)}.`);
    if(safe(snap.avg)<safe(snap.refs?.avg)&&safe(snap.totalEval)>0)improvements.push(`Trabalhar a experiência do cliente para superar a referência de nota média do Squad (${safe(snap.refs?.avg).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}).`);
    ...
    if(snap.previous&&safe(snap.points)<safe(snap.previous.points)*.95)improvements.push('Revisar os fatores que provocaram queda de pontuação em relação à competência anterior e definir uma correção de rota objetiva.');
  ...
  function feedbackAutoContent(generated={}){return{summary:generated.summary||'',strengths:generated.strengths||'',improvement_points:generated.improvement_points||'',next_month_goals:generated.next_month_goals||''}}
  function generatedFeedbackForTech(t,m=currentMonth(),squad=currentSquad()){
    const snap=feedbackSnapshotForTech(t,m,squad),generated=generatedFeedbackFromSnapshot(snap),autoContent=feedbackAutoContent(generated);
    return{...generated,generated_snapshot:{...snap,auto_content:autoContent}};
  }
  function feedbackFieldCanRefresh(currentValue,previousAutoValue){const current=String(currentValue||'').trim(),previous=String(previousAutoValue||'').trim();return!current||(!!previous&&current===previous)}
  function refreshedFeedbackPayloadForTech(t,generated,existing=null){
    if(!existing)return feedbackPayloadForTech(t,generated,null);
    const payload=feedbackPayloadForTech(t,generated,existing),newAuto=feedbackAutoContent(generated),oldSnapshot=existing.generated_snapshot||{};
    let oldAuto=oldSnapshot.auto_content||null;
    if(!oldAuto&&Object.keys(oldSnapshot).length)oldAuto=feedbackAutoContent(generatedFeedbackFromSnapshot(oldSnapshot));
    ['summary','strengths','improvement_points','next_month_goals'].forEach(field=>{payload[field]=feedbackFieldCanRefresh(existing[field],oldAuto?.[field])?newAuto[field]:existing[field]||'';});
    payload.manager_notes=existing.manager_notes||'';
    payload.generated_snapshot={...(generated.generated_snapshot||{}),auto_content:newAuto};
    payload.status=existing.status||'draft';payload.visible_to_technician=existing.visible_to_technician===true;payload.finalized_at=existing.finalized_at||null;payload.created_by=existing.created_by||payload.created_by;
    return payload;
  }
  ...
  function renderFeedbacks(){
    if(!isAdmin()||!$('#view-feedbacks'))return;const specific=state.squadCode!=='all',m=currentMonth(),squad=currentSquad();$('#feedbackScopeEmpty').classList.toggle('hidden',specific);$('#feedbackContent').classList.toggle('hidden',!specific);$('#generateSquadFeedbacksBtn').disabled=!specific||!m;if(!specific)return;
  ...
  async function generateSquadFeedbacks(){
    if(!isAdmin()||!requireSpecificSquad())return;const m=currentMonth(),squad=currentSquad();if(!m)return toast('Selecione uma competência com dados.');const key=feedbackScopeKey(squad,m),rows=await ensureFeedbacksLoaded(),techs=feedbackMonthTechs(m);if(!techs.length)return toast('Não há técnicos com produção nesta competência.');const btn=$('#generateSquadFeedbacksBtn');btn.disabled=true;const old=btn.textContent;btn.textContent='Atualizando...';try{const payloads=techs.map(t=>{const existing=feedbackRowForTech(rows,t.name),generated=generatedFeedbackForTech(t,m,squad);return refreshedFeedbackPayloadForTech(t,generated,existing)});const saved=await persistGeneratedFeedbacks(payloads);const outsideScope=rows.filter(r=>!techs.some(t=>samePersonName(t.name,r.technician_name)));state.feedbackCache[key]=state.supabase?[...outsideScope,...saved]:saved;toast(`Dados automáticos de ${techs.length} feedback(s) atualizados. Conteúdo escrito pelo gestor foi preservado.`);renderFeedbacks();}catch(err){console.error(err);toast('Não foi possível atualizar os feedbacks. Confira a migração V2.25.0.');}finally{btn.disabled=false;btn.textContent=old;}}
  ...
  async function confirmCsvImport(){
    ...
    try{
      ...
      if(pending.scopeAll){
        ...
        saveDemoSquads();state.financeRankingCache={};if(state.analysisPreset==='month')resetAnalysisRange(true);refreshSelectors();render();state.pendingCsv=null;closeModal('importModal');toast(`${MONTHS_PT[month-1]} ${year}: ${importedSquads} Squads e ${importedTechs} técnicos atualizados.`);
      }else{
        const s=currentSquad(),previous=s.months[id];if(previous?.isClosed)throw new Error(`${MONTHS_PT[month-1]} ${year} está fechado no Squad ${state.squadCode}. Reabra o mês antes de importar.`);const data=buildMonthFromCsv(pending.rows,id,pending.fileName,previous,state.squadCode);s.months[id]=data;state.currentId=id;state.techName=data.technicians[0]?.name||'';saveDemoSquads();if(state.analysisPreset==='month')resetAnalysisRange(true);
