(() => {
  ...
  function applyPermissions(){
    ...
    const analyticalView=['individual','team','indicators'].includes(state.currentView);if($('.month-control'))$('.month-control').classList.toggle('hidden',analyticalView||costAdminView||['users','profile','help','my-feedbacks'].includes(state.currentView));if($('#analysisDateControl'))$('#analysisDateControl').classList.toggle('hidden',!['individual','team'].includes(state.currentView));
  ...
  function groupCountBadgeHtml(excluded,{compact=false}={}){
    if(!excluded)return'';const title='Produção mantida nos totais, mas técnico desconsiderado no divisor das referências do grupo nesta competência.';
  ...
  function showView(name,adminSection=null){
    ...
    $('.month-control').classList.toggle('hidden',analytical||costAdminView||name==='users'||name==='profile'||name==='my-feedbacks'||name==='help');
  ...
  function renderTeam(){
    ...
    $('#teamAvgPoints').textContent=fmtNum(avgPoints);$('#teamAvgPointsNote').textContent='Pontuação média do período • não define o status da equipe';$('#teamStatusAudit').textContent=teamStatus.count?`${aboveCount} de ${teamStatus.count} técnicos considerados com status ACIMA • ${fmtPct(ratio)} • mínimo 50%${teamStatus.excludedCount?` • ${teamStatus.excludedCount} fora do divisor`:''}`:'Sem técnicos classificados no período';
...
function businessDaysMonthSummary(squads,id,cutoff){
  ...
  for(const squad of squads||[]){const mon=squad?.months?.[id];if(!mon)continue;if(!serviceDailyCoverage(mon).complete)dailyAligned=false;availableDays=Math.max(availableDays,businessDayCalendar(y,mn,Math.min(cutoffDay,safe(mon.latestDay)||cutoffDay)).length);const status=businessDaysStatusForSquad(mon,cutoffDay);above+=status.above;below+=status.below;for(const t of mon.technicians||[]){let has=false;for(const d of t.daily||[]){if(safe(d.day)>cutoffDay||safe(d.day)>safe(mon.latestDay)||d.off)continue;const date=isoDateParts(y,mn,safe(d.day));if(!isBusinessDateIso(date))continue;service.att+=safe(d.att);service.notes5+=safe(d.notes5);service.notes4+=safe(d.notes4);service.notes3+=safe(d.notes3);service.notes2+=safe(d.notes2);service.notes1+=safe(d.notes1);if(safe(d.att)+qualityNoteTotal(d)>0)has=true;}if(has&&!t.excludeFromGroupCount)techs.add(`${squad.code}|${nameLinkKey(t.name)}`);}for(const q of qualityRowsForMonth(mon)){if(safe(q.day)>cutoffDay||!isBusinessDateIso(qualityRowDate(mon,q)))continue;const target=q.qualityType==='product'?product:q.qualityType==='company'?company:null;if(!target)continue;target.notes5+=safe(q.notes5);target.notes4+=safe(q.notes4);target.notes3+=safe(q.notes3);target.notes2+=safe(q.notes2);target.notes1+=safe(q.notes1);}}
...
function renderIndicatorLineChart(el,labels,series,{maxValue=null,percent=false,decimals=0,height=340,fitWidth=false,emphasis=false,axisFormatter=null,valueFormatter=null}={}){
  ...
  const paths=validSeries.map((s,si)=>{
    ...
    const dots=(s.values||[]).map((v,i)=>{if(v==null)return'';const vacation=!!s.vacations?.[i],cx=x(i),cy=y(v);return `${vacation?`<circle cx="${cx}" cy="${cy}" r="${pointRadius+4.2}" fill="none" stroke="var(--warn)" stroke-width="2.2" class="vacation-point-halo chart-series-shape" data-series-index="${si}" data-point-index="${i}"></circle>`:''}<circle cx="${cx}" cy="${cy}" r="${vacation?pointRadius+0.8:pointRadius}" fill="${color}" class="chart-point chart-series-shape${vacation?' vacation-point':''}" style="--series-color:${color}" data-series-index="${si}" data-point-index="${i}"></circle>`}).join('');
  ...
  function renderScoreSettings(m){
    ...
    const pop=scoreReferencePopulation(m?.technicians||[]),countInfo=pop.active.length?` • ${pop.counted.length||pop.active.length} de ${pop.active.length} técnicos no divisor${pop.excludedCount?` (${pop.excludedCount} desconsiderado${pop.excludedCount>1?'s':''})`:''}`:'';
    $('#scoreAutoHint').innerHTML=m?.isClosed?`🔒 <strong>Médias congeladas no fechamento:</strong> ${fmtNum(rules.refAtt)} atend. • ${fmtNum(rules.refTotalEval)} avaliações • nota ${safe(rules.refAvg).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})} • ${fmtPct(rules.refEvalPct)}${countInfo}.`:`<strong>Médias atuais do Squad:</strong> ${fmtNum(rules.refAtt)} atend. • ${fmtNum(rules.refTotalEval)} avaliações • nota ${safe(rules.refAvg).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})} • ${fmtPct(rules.refEvalPct)}${countInfo}. Produção dos desconsiderados continua nos totais.`;
  }
  ...
  function renderMonthlyMetrics(m){
    ...
    const pop=scoreReferencePopulation(m?.technicians||[]),excludedEvalTotal=(m?.technicians||[]).reduce((s,t)=>s+normalizedEvaluationExcludedAtt(t),0);$('#monthlyMetricsHint').textContent=locked?`${m.monthName} ${m.year} • 🔒 mês fechado: metas, ajustes e pontuação estão congelados.`:`${m.monthName} ${m.year} • ${pop.counted.length||pop.active.length} de ${pop.active.length} técnicos compõem as médias • ${fmtInt(excludedEvalTotal)} atend. sem avaliação descontados da base das taxas.`;
  ...
  async function saveMonthlyMetrics(){
    ...
    try{
      ...
      recalculateMonth(m);saveDemoSquads();if(state.supabase){await persistManualMetrics(m);await persistFinanceMonth(m)}refreshSelectors();render();toast('Metas e atendimentos sem avaliação salvos. Taxas, status, ranking e bonificação recalculados.');
  ...
  function scoreReferencePopulation(rows){
    ...
    const counted=active.filter(t=>!t.excludeFromGroupCount);
    const ratioBase=counted.length?counted:active;
    const divisor=counted.length||active.length||1;
    return{active,counted,ratioBase,divisor,excludedCount:Math.max(0,active.length-counted.length)};
  }
  function scoreRefsFromRows(rows){
    const pop=scoreReferencePopulation(rows);
    // Regra operacional para tecnicos desconsiderados na quantidade do grupo:
    // - atendimentos e avaliacoes deles permanecem no numerador do Squad;
    // - eles nao compoem o divisor das referencias quantitativas;
    // - nota media e % avaliado usam apenas os tecnicos considerados, evitando inflar proporcoes.
    ...
    return{
      ...
      refAvg:truncate2(meanOf(pop.ratioBase,t=>t.avg)),
      refEvalPct:roundTo(meanOf(pop.ratioBase,t=>t.evalPct),4),
      activeCount:pop.active.length,
      countedCount:pop.counted.length||pop.active.length,
      excludedCount:pop.excludedCount
    };
  }
  function automaticScoreRefs(m){
    // Replica a formula oficial de status (2 de 4 = ACIMA) e aplica o checkbox
    // "Desconsiderar na quantidade de tecnicos do grupo" somente na formacao das referencias.
  ...
  function scoreRules(m,options={}){
    ...
    return{refAtt:auto.refAtt,refTotalEval:auto.refTotalEval,refAvg:auto.refAvg,refEvalPct:auto.refEvalPct,baseRefAtt:auto.refAtt,baseRefTotalEval:auto.refTotalEval,bonusAtt:safe(saved.bonusAtt)||20,bonusTotalEval:safe(saved.bonusTotalEval)||30,bonusAvg:safe(saved.bonusAvg)||40,bonusEvalPct:safe(saved.bonusEvalPct)||35,manualCount:0,progress:1,elapsedDays,totalDays,activeCount:auto.activeCount||0,countedCount:auto.countedCount||0,excludedCount:auto.excludedCount||0,attSource:'Total do Squad ÷ técnicos considerados (0 casas)',evalSource:'Avaliações do Squad ÷ técnicos considerados (0 casas)',avgSource:'Média dos técnicos considerados truncada (2 casas)',pctSource:'Média da % ajustada dos técnicos considerados (4 casas)'};
  }
  ...
  function teamStatusFromTechnicianStatuses(rows,{period=false}={}){
    ...
    // O resultado da equipe nao usa pontuacao media. Ele usa o mesmo status ACIMA/ABAIXO
    // de cada tecnico (2 de 4 criterios). Quem estiver marcado como "desconsiderar na quantidade
    // de tecnicos do grupo" mantem producao/status, mas nao entra no denominador da equipe.
    const counted=active.filter(t=>!t.excludeFromGroupCount),base=counted.length?counted:active;
    const readStatus=t=>String(period?(t.periodStatus||t.status||''):(t.status||t.periodStatus||'')).toUpperCase();
    const classified=base.filter(t=>['ACIMA','ABAIXO'].includes(readStatus(t)));
    ...
    return{status:count?(ratio>=.5?'ACIMA':'ABAIXO'):'',aboveCount,belowCount,count,ratio,excludedCount:Math.max(0,active.length-base.length)};
  }
  ...
  function buildFinanceModelData({mode,hasProduction,days,avgPerDay,notes5Pct,eligibleAtt,evaluationExcludedAtt,commissionAtt,commissionNotes5,cancelRate,cancelTier,rawMult,effectiveMult,financeStatus,topAttBonus,topNotes5Bonus,manualBonus,sales,discount,redistributed,vacation,pool}){
  ...
  function recalculateFinance(m){
    ...
    for(const t of m.technicians||[]){
      ...
      const common={hasProduction,days,eligibleAtt,evaluationExcludedAtt,cancelRate,cancelTier,rawMult,effectiveMult,financeStatus,topAttBonus,topNotes5Bonus,manualBonus,sales,discount,redistributed,vacation:!!t.vacation,pool};
  ...
  async function saveFinanceTechnicians(){
    ...
    for(const row of $$('#financeTechnicianRows [data-finance-tech]')){const t=m.technicians.find(x=>samePersonName(x.name,row.dataset.financeTech));if(!t)continue;for(const input of $$('[data-finance-field]',row)){if(['vacation','excludeFromGroupCount'].includes(input.dataset.financeField))t[input.dataset.financeField]=!!input.checked;else t[input.dataset.financeField]=safe(input.value)}t.evaluationExcludedAtt=normalizedEvaluationExcludedAtt(t)}
    try{recalculateMonth(m);saveDemoSquads();if(state.supabase)await persistFinanceMonth(m);state.financeRankingCache={};render();toast('Valores salvos; referências, status, ranking e bonificação foram recalculados.')}catch(err){console.error(err);toast('Não foi possível salvar os valores financeiros.')}
  }
