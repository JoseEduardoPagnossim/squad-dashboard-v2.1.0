...
function setIndicatorSection(section){
  state.indicatorSection=section==='quality'?'quality':'performance';
  if(state.currentView==='indicators')renderIndicators();
}
function syncIndicatorSectionUi(){
  const quality=state.indicatorSection==='quality';
  $$('#view-indicators [data-indicator-section]').forEach(btn=>{const active=btn.dataset.indicatorSection===state.indicatorSection;btn.classList.toggle('active',active);btn.setAttribute('aria-selected',active?'true':'false')});
  if($('#indicatorPerformancePanel'))$('#indicatorPerformancePanel').classList.toggle('hidden',quality);
  if($('#indicatorQualityPanel'))$('#indicatorQualityPanel').classList.toggle('hidden',!quality);
  if($('#indicatorPerformanceActions'))$('#indicatorPerformanceActions').classList.toggle('hidden',quality);
  if($('#indicatorHeroTitle'))$('#indicatorHeroTitle').textContent=quality?'Indicadores de qualidade':'Indicadores gerais';
  if($('#indicatorHeroText'))$('#indicatorHeroText').textContent=quality?'Serviço, Produto e Empresa analisados separadamente, preservando a origem de cada avaliação e os gráficos usados na reunião semanal.':'Leitura consolidada para o Admin geral com calendário diário, visão semanal/mensal, produtividade por tempo trabalhado e histórico comparativo entre técnicos ou Squads.';
}
...
function qualitySourceInfo(squads,rangeIds){
  const files=new Set(),dates=[];for(const squad of squads||[])for(const id of rangeIds||[]){const m=squad?.months?.[id];if(!m)continue;for(const t of m.technicians||[])for(const q of t.qualityDaily||[]){if(q.sourceFile)files.add(q.sourceFile);if(q.importedAt)dates.push(q.importedAt)}}
...
function renderIndicatorLineChart(el,labels,series,{maxValue=null,percent=false,decimals=0,height=340,fitWidth=false,emphasis=false}={}){
  ...
  async function handleCsvFile(e){
    ...
    try{
      ...
      if(kind==='quality'){
        ...
        $('#importDetails').innerHTML=`<strong>${fmtInt(mapped.rows.length)} avaliações vinculadas</strong> • <strong>${months.length} meses disponíveis</strong>${un}${amb} • ${fmtInt(parsed.ignored)} linha(s) inválida(s)/sem nota de Produto ou Empresa. <span class="muted">Clientes não são necessários. Resolve, comentário e NotaServico não entram nos cálculos.</span>`;$('#importProgress').style.width='100%';return;
      }
  ...
  function qualityTechnicianCandidates(row,codes){
    const aliasKeys=qualityUserAliasKeys(row.name),found=[];for(const code of codes||[]){const m=state.squads?.[code]?.months?.[row.id];if(!m)continue;for(const t of m.technicians||[])if(aliasKeys.has(nameLinkKey(t.name)))found.push({group:code,name:t.name,tech:t,month:m});}const dedupe=new Map(found.map(x=>[`${x.group}|${nameLinkKey(x.name)}`,x]));return[...dedupe.values()];
  }
  function mapQualityCsvRows(rows,codes){const mapped=[],unmatchedCounts=new Map(),ambiguousCounts=new Map();for(const row of rows||[]){const candidates=qualityTechnicianCandidates(row,codes);if(candidates.length===1){mapped.push({...row,group:candidates[0].group,name:candidates[0].name})}else if(!candidates.length)unmatchedCounts.set(row.name,(unmatchedCounts.get(row.name)||0)+1);else ambiguousCounts.set(row.name,(ambiguousCounts.get(row.name)||0)+1)}return{rows:mapped,unmatched:[...unmatchedCounts.keys()].map(titleWords).sort((a,b)=>a.localeCompare(b,'pt-BR')),ambiguous:[...ambiguousCounts.keys()].map(titleWords).sort((a,b)=>a.localeCompare(b,'pt-BR'))}}
  function applyQualityMonthImport(m,rows,fileName){
    const now=new Date().toISOString(),grouped=new Map();for(const row of rows||[]){const key=`${nameLinkKey(row.name)}|${row.day}`,g=grouped.get(key)||{name:row.name,day:row.day,product:{notes5:0,notes4:0,notes3:0,notes2:0,notes1:0},company:{notes5:0,notes4:0,notes3:0,notes2:0,notes1:0}};if(row.productNote)g.product[`notes${row.productNote}`]++;if(row.companyNote)g.company[`notes${row.companyNote}`]++;grouped.set(key,g)}
    for(const t of m.technicians||[]){t.qualityDaily=(t.qualityDaily||[]).filter(q=>!['product','company'].includes(q.qualityType));for(const g of grouped.values()){if(!samePersonName(g.name,t.name))continue;for(const type of ['product','company']){const counts=g[type];if(!qualityNoteTotal(counts))continue;t.qualityDaily.push({day:g.day,qualityType:type,...counts,sourceFile:fileName,importedAt:now})}}t.qualityDaily.sort((a,b)=>safe(a.day)-safe(b.day)||String(a.qualityType).localeCompare(String(b.qualityType)));}
    return{sourceRows:(rows||[]).length,dailyRows:[...grouped.values()].length};
  }
  async function persistQualityMonth(m,fileName){
    if(!state.supabase)return;const inserts=[];for(const t of m.technicians||[]){if(!t.dbId)continue;const {error:de}=await state.supabase.from('quality_daily_metrics').delete().eq('technician_month_id',t.dbId);if(de)throw de;for(const q of t.qualityDaily||[])inserts.push({technician_month_id:t.dbId,day:safe(q.day),quality_type:q.qualityType,notes5:safe(q.notes5),notes4:safe(q.notes4),notes3:safe(q.notes3),notes2:safe(q.notes2),notes1:safe(q.notes1),source_file:q.sourceFile||fileName,imported_by:state.user.userId,imported_at:q.importedAt||new Date().toISOString()});}
    for(let i=0;i<inserts.length;i+=500){const {error}=await state.supabase.from('quality_daily_metrics').insert(inserts.slice(i,i+500));if(error)throw error}
  }
