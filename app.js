...
function renderIndicatorLineChart(el,labels,series,{maxValue=null,percent=false,decimals=0,height=340,fitWidth=false,emphasis=false}={}){
  ...
  function recalculateMonth(m,options={}){
    ...
    if(m.isClosed&&Array.isArray(m.closedSnapshot?.technicians)){
      ...
      if(typeof m.closedSnapshot.financeCompare==='boolean')m.financeCompare=m.closedSnapshot.financeCompare;if(m.closedSnapshot.financeComparison)m.financeComparison=clone(m.closedSnapshot.financeComparison);
  ...
  function buildFinanceModelData({mode,hasProduction,days,avgPerDay,notes5Pct,commissionAtt,commissionNotes5,cancelRate,cancelTier,rawMult,effectiveMult,financeStatus,topAttBonus,topNotes5Bonus,manualBonus,sales,discount,redistributed,vacation,pool}){
    ...
    const beforeVacation=hasProduction?Math.max(0,afterCancel+manualBonus+topAttBonus+topNotes5Bonus+sales-discount+redistributed):0,final=vacation?beforeVacation*.5:beforeVacation;
    return{mode,hasProduction,days,avgPerDay,notes5Pct,attendanceTier:null,notes5Tier:null,commissionAtt:safe(commissionAtt),commissionNotes5:safe(commissionNotes5),base,cancelRate,cancelTier:safe(cancelTier?.max),cancelRawMultiplier:rawMult,cancelMultiplier:effectiveMult,afterCancel,financeStatus,topAttBonus,topNotes5Bonus,manualBonus,salesCommission:sales,discount,redistribution:redistributed,beforeVacation,vacation:!!vacation,vacationFactor:vacation?.5:1,final:Number(final.toFixed(2)),pool:Number(pool.toFixed(2))}
  }
  function recalculateFinance(m){
    if(!m||m.isClosed)return;
    const settings=financeSettingsForMonth(m);m.financeSettings=settings;m.financeMonthData=m.financeMonthData||{};m.financeModel=financeModelForMonth(m);if(typeof m.financeCompare!=='boolean')m.financeCompare=true;
    ...
    const active=(m.technicians||[]).filter(t=>safe(t.att)>0),days=Math.max(1,businessDaysElapsed(m.year,m.month,m.latestDay));
    const totalAtt=active.reduce((s,t)=>s+safe(t.att),0),totalN5=active.reduce((s,t)=>s+safe(t.notes5),0),groupAvgPerDay=active.length?totalAtt/(days*active.length):0,groupNotes5Pct=totalAtt?totalN5/totalAtt:0;
    ...
    let squadTotal=0,individualTotal=0;
    for(const t of m.technicians||[]){
      ...
      squadTotal+=squad.final;individualTotal+=individual.final;const official=m.financeModel==='individual'?individual:squad;
      t.financeData={...official,version:2,officialModel:m.financeModel,models:{squad,individual},comparisonDiff:Number((individual.final-squad.final).toFixed(2)),groupBase:{avgPerDay:groupAvgPerDay,notes5Pct:groupNotes5Pct,commissionAtt:groupCommissionAtt,commissionNotes5:groupCommissionNotes5,afterCancel:groupBase}};
    }
    const diff=individualTotal-squadTotal;m.financeComparison={version:1,squadTotal:Number(squadTotal.toFixed(2)),individualTotal:Number(individualTotal.toFixed(2)),difference:Number(diff.toFixed(2)),differencePct:squadTotal?diff/squadTotal:0,groupAvgPerDay,groupNotes5Pct,groupCommissionAtt,groupCommissionNotes5,groupAfterCancel:groupBase,cancelRate,cancelMultiplier:effectiveMult,activeTechnicians:active.length};m.financeComparisonSnapshot=clone(m.financeComparison);
  }
  function renderFinanceSummary(t,m){
    ...
    const compare=m.financeCompare&&otherData?` • Simulação ${financeModelLabel(other)}: ${fmtMoney(otherData.final)}.`:'';$('#financeSummarySubtitle').textContent=(t.vacation?`Subtotal antes das férias: ${fmtMoney(d.beforeVacation)} • redutor final de 50%.`:`Modelo oficial: ${financeModelLabel(model)}.`)+compare;
  }
  ...
  function renderFinanceAdmin(m,specific,locked){
    ...
    $('#financeModelSquad').checked=model==='squad';$('#financeModelIndividual').checked=model==='individual';$('#financeModelSquad').disabled=locked;$('#financeModelIndividual').disabled=locked;$('#financeCompareToggle').checked=m.financeCompare!==false;$('#financeCompareToggle').disabled=locked;
  ...
  async function saveFinanceConfiguration(){
    ...
    try{m.financeModel=$('#financeModelIndividual')?.checked?'individual':'squad';m.financeCompare=$('#financeCompareToggle')?.checked!==false;m.financeSettings=collectFinanceSettingsFromUi(m);m.financeMonthData={customersStart:Math.max(0,safe($('#financeCustomersStart').value)),canceledCount:Math.max(0,safe($('#financeCanceledCount').value))};recalculateMonth(m);saveDemoSquads();if(state.supabase)await persistFinanceMonth(m);render();toast(`Modelo ${financeModelLabel(m.financeModel)} salvo como oficial e bonificação recalculada.`)}catch(err){console.error(err);toast('Não foi possível salvar. Confira se a migração V2.19.0 foi executada.')}
  }
  async function saveFinanceTechnicians(){
    ...
    try{recalculateMonth(m);saveDemoSquads();if(state.supabase)await persistFinanceMonth(m);render();toast('Valores individuais salvos; os dois modelos foram recalculados.')}catch(err){console.error(err);toast('Não foi possível salvar os valores financeiros.')}
  }
  async function persistFinanceMonth(m){
    if(!state.supabase||!m?.dbId)return;const payload={finance_settings:financeSettingsForMonth(m),finance_month_data:m.financeMonthData||{},finance_model:financeModelForMonth(m),finance_compare:m.financeCompare!==false,finance_comparison_snapshot:m.financeComparison||{}};const {error:me}=await state.supabase.from('squad_months').update(payload).eq('id',m.dbId);if(me)throw me;
  ...
  async function closeMonth(id){
    ...
    }catch(err){console.error(err);toast('Não foi possível fechar o mês. Confira as migrações V2.4.0, V2.18.0 e V2.19.0.')}
  }
